"""
Kinematic controller with floating base for H1-2 robot handless model.

It moves the left wrist to a desired target pose using an iterative
damped least squares approach. It also publishes the joint states for
visualization and broadcasts the TF from the world frame to the pelvis
frame ("floating base").
...
class IKTestNode(Node):
    ...
    def __init__(self):
        super().__init__('h1_2_kinematic_controller_wb1')
    ...
    def update(self):
        ...
        JJt = J @ J.T
        dq = J.T @ np.linalg.solve(JJt + self.damping*np.eye(6), error)
