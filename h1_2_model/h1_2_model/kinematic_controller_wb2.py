"""
Kinematic controller with floating base and both feet fixed on the ground.

This controller implements inverse kinematics for the H1-2 humanoid robot,
constraining both feet to remain fixed on the ground while controlling the
left hand end-effector position using a damped least-squares method. It uses
a stacked Jacobian to include the foot constraints in the IK solution.

Run it as: ros2 launch h1_2_model display_handless_kinecontrol.launch.py
           ros2 run h1_2_model kinematic_controller_wb2
"""
...
class IKTestNode(Node):
    ...
    def __init__(self):
        super().__init__('h1_2_kinematic_controller_wb2')
        ...
        # Feet frames
        self.left_foot_frame  = "left_ankle_roll_link"
        self.right_foot_frame = "right_ankle_roll_link"
        self.lf_id = self.model.getFrameId(self.left_foot_frame)
        self.rf_id = self.model.getFrameId(self.right_foot_frame)
        ...
        p_target = np.array([0.4+0.45, 0.25+0.30, 1.1+0.35])
    ...
    def update(self):
        ...
        # Desired twist vector
        error_extended = np.hstack([error,          # 6D hand task
                                    np.zeros(6),    # left foot fixed
                                    np.zeros(6)     # right foot fixed
        ])
        ...
        J_hand = pin.getFrameJacobian(self.model, self.data, self.ee_id, 
                                      pin.ReferenceFrame.LOCAL_WORLD_ALIGNED)

        J_lf = pin.getFrameJacobian(self.model, self.data, self.lf_id,
                                    pin.ReferenceFrame.LOCAL_WORLD_ALIGNED)

        J_rf = pin.getFrameJacobian(self.model, self.data, self.rf_id,
                                    pin.ReferenceFrame.LOCAL_WORLD_ALIGNED)

        # Stacked Jacobian
        J = np.vstack([J_hand, J_lf, J_rf])

        # Use the full Jacobian (including floating base columns)
        JJt = J @ J.T
        dq = J.T @ np.linalg.solve(JJt + self.damping*np.eye(JJt.shape[0]), error_extended)

        # Integrate base and actuated joints
        self.q = pin.integrate(self.model, self.q, self.alpha*dq)
